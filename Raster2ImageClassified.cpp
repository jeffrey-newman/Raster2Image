...
//            ("png,p", po::value<std::string>(&png_file), "file path to save the png")
            ("style,s", po::value<int>(&colours), "Specify 0 for LANDUSE1, 1 for LANDUSE2 colour schemes")
    ...
    boost::shared_ptr<MagickWriterClassified> writer(nullptr);
    if (colours == LANDUSE1) writer.reset(new MagickWriterClassified(LANDUSE1));
    if (colours == LANDUSE2) writer.reset(new MagickWriterClassified(LANDUSE2));
